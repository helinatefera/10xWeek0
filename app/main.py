...
st.markdown(
    ...
    .stAppHeader{
        background-color: rgba(216, 171, 65, 0.3);
    }
    .stMain{
...
with st.sidebar:
    selected_option = st.radio("Select an option:", ["Display head of CSV file", "Plot Histogram","Summary Statistics","Data Quality Check","Correlation Analysis","Wind Analysis","Temperature Analysis"])
...
if selected_option == "Display head of CSV file":
    ...
    data = pd.read_csv(uploaded_file)   
    wind_speed_col = st.selectbox("Select Wind Speed Column", data.columns, index=data.columns.get_loc('WS'))
    wind_direction_col = st.selectbox("Select Wind Direction Column", data.columns, index=data.columns.get_loc('WD'))

    bins = st.slider("Number of Bins for Wind Speed", min_value=4, max_value=12, value=8, step=1)
    
    if st.button("Generate Wind Rose"):
        plot_wind_rose(data, wind_speed_col=wind_speed_col, wind_direction_col=wind_direction_col, bins=bins)
elif selected_option == "Temperature Analysis":
    data = pd.read_csv(uploaded_file)
    rh_col = st.selectbox("Select Relative Humidity Column", data.columns, index=data.columns.get_loc('RH'))
    temp_cols = st.multiselect("Select Temperature Columns", data.columns, default=["TModA", "TModB"])
    solar_cols = st.multiselect("Select Solar Radiation Columns", data.columns, default=["GHI", "DNI", "DHI"])
    
    if st.button("Analyze"):
        temperature_humidity_analysis(data, rh_col=rh_col, temp_cols=temp_cols, solar_cols=solar_cols)
