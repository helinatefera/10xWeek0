...
def temperature_humidity_analysis(
    data,
    rh_col="RH",
    temp_cols=["TModA", "TModB"],
    ...
    """
    st.subheader(
        "Temperature and Humidity Analysis (Alternative Visualizations)"
    )
...
def dataset_summary(data):
    ...
    if duplicate_count > 0:
        st.warning(
            f"{duplicate_count} duplicate rows found! Consider removing them."
        )
    ...
    selected_columns = st.multiselect(
        "Select columns to check for negative or zero values", numeric_columns
    )
    if selected_columns:
        for col in selected_columns:
            ...
            if not negative_or_zero.empty:
                st.warning(
                    f"Column `{col}` has {len(negative_or_zero)} rows with negative or zero values."
                )
...
def correlation_analysis(data):
    ...
    st.write(
        "Investigate pairwise relationships among `GHI`, `DNI`, `DHI`, `TModA`, and `TModB`."
    )
    pairplot_fig = sns.pairplot(
        data[solar_columns], diag_kind="kde", corner=True
    )
    ...
    y_var = st.selectbox(
        "Select Solar Irradiance Component (Y-axis)", solar_columns
    )
